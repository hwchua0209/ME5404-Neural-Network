...
gamma = 0.9; % 0.5, 0.9 (discount factor)
model = 5; % 1, 2, 3, 4, 5(additional)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for run = 1:10
    ...
    while trial < 3001 
        ...
        while state_k ~= 100
            ...
            [epsilon, alpha] = get_param(model, k);
...
[t_1,s_1] = title({'Action Selected by Optimal Policy for Each State';...
    ['gamma = ', num2str(gamma), ', Model = ', num2str(model)]});
ax = gca;
...
figure(2);
grid on; grid minor; hold on; axis ij;
[t_2,s_2] = title({'Optimal Path Taken by Robot';...
    ['gamma = ', num2str(gamma), ', Model = ', num2str(model), ', Rewards = ', num2str(opt_max_rew)]});
ax = gca;
ax.GridAlpha = 0; ax.LineWidth = 0.6;
ax.MinorGridColor = 'b'; ax.MinorGridAlpha = 1; ax.MinorGridLineStyle = '-';
ax.XLim = [0,11]; ax.YLim = [0,11];
